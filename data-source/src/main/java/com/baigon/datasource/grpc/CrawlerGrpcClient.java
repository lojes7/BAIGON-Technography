...
// 注：gateway 已提供 crawler 查询的 REST 端点，但 data-source 是服务间直连，
...
public class CrawlerGrpcClient {
     ...
     * 返回 null 表示未找到或服务不可用。
     */
    public GetJobSourceByTraceIdResponse getJobSourceByTraceId(long traceId) {
        ...
        try {
            ...
            GetJobSourceByTraceIdResponse resp = stub.getJobSourceByTraceId(
                    GetJobSourceByTraceIdRequest.newBuilder().setTraceId(traceId).build());
            return resp;
        } catch (Exception e) {
