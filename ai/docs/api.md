...
Pydantic 严格结构校验，再对每个非空名称、描述和日期执行确定性原文匹配；描述按行校验，
仅忽略 PDF/OCR 排版产生的空白差异，并只保留能在原文中找到的行。其他找不到来源的字段会被
清空，没有可确认身份字段的整条记录会被移除。熟练度只有在技能所在原文分句中
出现明确依据时才可填写，非空值只能是 `Basic`、`Familiar`、`Advanced`、`Expert`。
日期保留原文已有精度：仅年份使用 `YYYY`，年月使用 `YYYY-MM`，完整日期使用
`YYYY-MM-DD`，没有日期时使用空字符串；服务不会为缺失的月份或日期补默认值。
...
非法熟练度、重复技能和超长结果，再确定性检查每条 `evidence` 是否能在简历原文中连续定位；
匹配仅忽略 PDF/OCR 排版空白，无法回溯原文的单项技能会被过滤，不会导致整批分析失败。
