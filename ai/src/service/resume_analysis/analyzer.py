...
5. 日期按原文已有精度填写：仅有年份时使用 YYYY，有年月时使用 YYYY-MM，有完整日期时使用 YYYY-MM-DD；原文使用点号或斜杠时，只把分隔符改为连字符；原文没有日期时填写空字符串，禁止擅自补月或补日。
6. proficiency 非空时只能是 Basic、Familiar、Advanced、Expert；只有技能上下文明确出现对应熟练度时才能填写，否则必须为空字符串。
7. description 必须保留原文的换行或条目边界，每一行都逐字复制，不得把多行概括或改写成新句子。
8. 禁止返回解释、Markdown、普通正文或额外字段，只能调用 submit_resume_analysis 一次。
""".strip()
