...
function UsersPage() {
  ...
  const [staffForm, setStaffForm] = useState({ uid: "", name: "", password: "", role: "DATA_ANALYST" });
  ...
  const handleQuickCreateSchoolFromSA = async () => {
    ...
    setInlineDept(p => ({ ...p, creating: true }));
    try {
      await createSchool({ name: inlineDept.schoolName });
      toast.success("学院已创建");
      const sch = await getSchoolList();
      setSchools(sch.data);
  ...
  const handleCreateStaff = async () => {
    ...
    try {
      ...
      setStaffModal(false); setStaffForm({ uid: "", name: "", password: "", role: "DATA_ANALYST" });
      refreshAll();
    } catch (err) { toast.error((err as Error).message); }
  };
