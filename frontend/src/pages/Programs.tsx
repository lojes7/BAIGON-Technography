          ...
          <div className="flex items-start justify-between">
            <div>
              <div className="flex items-center gap-3 mb-1">
                <span className="text-[13px]" style={{ color: T.info }}>当前版本</span>
                <span className="font-mono text-[13px] font-medium" style={{ color: T.ink }}>2026版</span>
                <span style={{ color: T.info }}>·</span>
                <span className="text-[13px]" style={{ color: T.info }}>生效 2026-09</span>
              ...
              <div className="flex items-center gap-6 text-[13px] mt-2">
                {[
                  ...
                  <span key={i}><span style={{ color: T.info }}>{k} </span>
                    <span className="font-mono font-medium" style={{ color: T.ink }}>{v}</span></span>
                ))}
              </div>
            </div>
      ...
      <div className="flex items-center gap-1 border-b" style={{ borderColor: T.border }}>
        {tabs.map(tab => (
          <button key={tab}
            ...
            {tab}
            {activeTab === tab && (
              <div className="absolute bottom-0 left-0 right-0 h-0.5 rounded-full" style={{ background: T.teal }} />
            )}
          </button>
        ))}
      </div>
      ...
      {activeTab === "培养概览" && (
        <>
          <div className="grid grid-cols-3 gap-4">
            <MetricCard title="能力覆盖率" value="68%" sub="113 / 166 项能力有课程支撑"
              trend={{ label: "较上版本 +8%", up: true }} />
            <MetricCard title="有考核证据的能力" value="54" sub="占已覆盖能力 47.8%"
              severity="warning" />
            <MetricCard title="高需求未覆盖能力" value="12" sub="需优先补充" severity="critical" />
          </div>
            ...
            <div className="px-4 pb-4 pt-2">
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={coverageData2} layout="vertical" barSize={14}>
                  <CartesianGrid strokeDasharray="3 3" stroke={T.cloud} horizontal={false} />
                  <XAxis type="number" domain={[0, 100]} unit="%"
                    tick={{ fontSize: 11, fill: T.info }} tickLine={false} axisLine={false} />
                  <YAxis type="category" dataKey="cat"
                    tick={{ fontSize: 12, fill: T.ink }} tickLine={false} axisLine={false} width={70} />
                  ...
                  <Bar dataKey="pct" name="覆盖率" radius={[0, 3, 3, 0]}>
                    {coverageData2.map((d, i) => (
                      <Cell key={i} fill={d.pct >= 80 ? T.emerging : d.pct >= 60 ? T.stable : T.pending} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            ...
            <div className="divide-y" style={{ borderColor: T.cloud }}>
              {[
                ...
                <div key={i} className="flex items-start gap-3 px-4 py-3">
                  {p.level === "critical" ? <AlertCircle size={14} style={{ color: T.risk, marginTop: 1 }} /> :
                    p.level === "warning" ? <AlertTriangle size={14} style={{ color: T.pending, marginTop: 1 }} /> :
                      <Info size={14} style={{ color: T.info, marginTop: 1 }} />}
                  <span className="text-[13px]" style={{ color: T.ink }}>{p.msg}</span>
                </div>
              ))}
            </div>
          </Card>
              ...
              {[
                ...
                <tr key={i} className="hover:bg-gray-50 transition-colors" style={{ borderTop: `1px solid ${T.cloud}` }}>
                  ...
                  <td className="px-4 py-3">
                    <span style={{ color: row.quality === "高" ? T.emerging : row.quality === "中" ? T.pending : T.risk }}>
                      {row.quality}
                    </span>
                  </td>
                  <td className="px-4 py-3">
          ...
          <table className="w-full text-[13px]">
            ...
            <tbody>
              {[
                ...
                const qColor = row.quality === "高" ? T.emerging : row.quality === "中" ? T.stable :
                  row.quality === "低" ? T.pending : T.risk;
                return (
                  <tr key={i} className="hover:bg-gray-50 transition-colors" style={{ borderTop: `1px solid ${T.cloud}` }}>
                    <td className="px-4 py-3 font-medium" style={{ color: T.ink }}>{row.skill}</td>
                    <td className="px-4 py-3 text-[12px]" style={{ color: T.info }}>{row.cat}</td>
                    <td className="px-4 py-3">
                      <span className="text-[12px]" style={{ color: row.demand === "极高" || row.demand === "高" ? T.risk : T.pending }}>
                        {row.demand}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3 text-center">
                      {row.best !== "—" ? (
                        <span className="text-[11px] px-2 py-0.5 rounded font-medium"
                          style={{ color: row.best === "H" ? T.emerging : row.best === "M" ? T.stable : T.pending,
                            background: row.best === "H" ? `${T.emerging}18` : row.best === "M" ? `${T.stable}18` : `${T.pending}18` }}>
      ...
      {activeTab === "证据质量" && (
        <div className="flex flex-col gap-4">
          <div className="grid grid-cols-4 gap-4">
            <MetricCard title="有考核证据（H）" value="28" trend={{ label: "占覆盖能力 24.8%", up: true }} />
            <MetricCard title="有教学证据（M）" value="41" trend={{ label: "占覆盖能力 36.3%", up: true }} />
            <MetricCard title="仅大纲声明（L）" value="44" severity="warning" sub="证据最弱" />
            <MetricCard title="完全无覆盖" value="12" severity="critical" sub="含3项高需求" />
          </div>
            ...
            <div className="px-4 pb-4 pt-2">
              <ResponsiveContainer width="100%" height={200}>
                <BarChart layout="vertical" barSize={12}
                  ...
                  <CartesianGrid strokeDasharray="3 3" stroke={T.cloud} horizontal={false} />
                  <XAxis type="number" tick={{ fontSize: 11, fill: T.info }} tickLine={false} axisLine={false} />
                  <YAxis type="category" dataKey="cat" tick={{ fontSize: 12, fill: T.ink }} tickLine={false} axisLine={false} width={65} />
                  <Tooltip contentStyle={{ fontSize: 12, border: `1px solid ${T.border}`, borderRadius: 6 }} />
                  <Legend wrapperStyle={{ fontSize: 12 }} />
                  <Bar dataKey="H" name="实践/考核" fill={T.emerging} stackId="a" radius={[0,0,0,0]} />
                  <Bar dataKey="M" name="正式教学" fill={T.stable} stackId="a" />
                  <Bar dataKey="L" name="大纲声明" fill={T.pending} stackId="a" />
                  <Bar dataKey="none" name="无覆盖" fill={T.risk} stackId="a" radius={[0,3,3,0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </Card>
              ...
              <div key={i} className="px-5 py-4">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    <span className="font-mono text-[14px] font-medium" style={{ color: T.ink }}>{v.version}</span>
                ...
                <ul className="space-y-1">
                  {v.changes.map((c, j) => (
                    <li key={j} className="flex items-start gap-2 text-[13px]" style={{ color: T.ink }}>
                      <span style={{ color: T.teal, flexShrink: 0 }}>·</span>{c}
                    </li>
                  ))}
                </ul>
              </div>
