...
function RecommendationsPage() {
  ...
  const priorityColors: Record<string, string> = {
    high: T.risk, medium: T.pending, low: T.info,
  };
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-[12px] font-mono ml-2" style={{ color: T.info }}>
        ...
        return (
          <div key={priority}>
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-[12px]" style={{ color: T.info }}>{items.length} 条</span>
            </div>
                    ...
                    <div className="px-5 py-4">
                      <div className="flex items-start justify-between gap-4 mb-3">
                        <div className="flex items-center gap-2 flex-wrap">
                          <h3 className="text-[14px] font-medium" style={{ color: T.ink }}>{rec.title}</h3>
                          {rec.aiGenerated && (
                            <span className="inline-flex items-center gap-1 text-[11px] px-1.5 py-0.5 rounded"
                              style={{ background: "#EBF2FA", color: T.stable, border: `1px solid ${T.stable}25` }}>
                        ...
                        <div className="flex items-center gap-2 flex-shrink-0 text-[12px]" style={{ color: T.info }}>
                          <span>{rec.type}</span>
                      ...
                      <div className="grid grid-cols-2 gap-4 mb-4">
                        <div>
                          ...
                          <p className="text-[13px] leading-relaxed" style={{ color: T.ink }}>{rec.rationale}</p>
                        </div>
                        <div>
                          <div className="text-[11px] font-medium uppercase tracking-wider mb-1.5" style={{ color: T.info }}>
                            改进建议
                          </div>
                          <p className="text-[13px] leading-relaxed" style={{ color: T.ink }}>{rec.suggestion}</p>
                        </div>
                      </div>
                      <div className="flex items-center justify-between">
                        ...
                        {(isAccepted || isRejected) && (
                          <button className="text-[12px]" style={{ color: T.info }}
                            onClick={() => {
